...
use function Thorm\{button, el, text, concat, show, on, cls, eq, html, mod, read, state, inc, client, div, h2, p, span};
...
\$app = div([ cls('container mt-5') ], [
    ...
    p([], [text('A simple stateful counter that reacts to clicks and shows even/odd state.')]),
    button([
        cls('btn btn-warning'),
        on('click', inc(\$cnt, 1))
    ], [ text('Inc') ]),
    show(eq(mod(read(\$cnt), 2), 0), span([ cls('m-5 fs-5') ], [ text('Even!') ]))
]);
", true))]);
